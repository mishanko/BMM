...
for j in tqdm(range(J-1)):
    ...
    for m in range(1, M-1):
        ...
        for n in range(1, N-1):
            F = 1 / 2 * (v[m-1, n, j] - 2 * v[m, n, j] + v[m+1, n, j]) * 4 * tau / hx**2 + v[m, n, j] * 4 + 2 * tau * np.cos(x[n]) * np.sin(t[j] + tau / 2)
    ...
    for n in range(1, N-1):
        ...
        for m in range(1, M-1):
            F = 1 / 2 * (w[m, n-1] - 2 * w[m, n] + w[m, n+1]) * 4 * tau / hy**2 + w[m, n] * 4 + 2 * tau * np.cos(x[n]) * np.sin(t[j] + tau / 2) 
